...
class MyPolicy(ModelPolicy):
    ...
    def load_model(self, checkpoint_path: str, device: str) -> Any:
        ...
        self._episode_initial_state14 = None
    ...
    def _relative_actions_to_absolute(self, actions14: np.ndarray) -> np.ndarray:
        if self._episode_initial_state14 is None:
            raise RuntimeError("Episode initial state is not set.")
        if actions14.ndim != 2 or actions14.shape[1] != 14:
            raise ValueError(f"Expected action shape (T, 14), got {actions14.shape}")

        init = self._episode_initial_state14.astype(np.float32)
        out = actions14.astype(np.float32).copy()

        # Translation is predicted relative to the episode start pose.
        out[:, 0:3] += init[0:3]
        out[:, 7:10] += init[7:10]

        # Rotation is predicted relative to the episode start orientation.
        left_init_rot = Rotation.from_euler("xyz", init[3:6].astype(np.float64))
        right_init_rot = Rotation.from_euler("xyz", init[10:13].astype(np.float64))
        left_rel_rot = Rotation.from_euler("xyz", actions14[:, 3:6].astype(np.float64))
        right_rel_rot = Rotation.from_euler("xyz", actions14[:, 10:13].astype(np.float64))

        out[:, 3:6] = (left_rel_rot * left_init_rot).as_euler("xyz").astype(np.float32)
        out[:, 10:13] = (right_rel_rot * right_init_rot).as_euler("xyz").astype(np.float32)

        # Keep gripper channels as-is.
        return out
    ...
    def _start_new_rollout(self, frame: Dict[str, Any], prompt: str) -> np.ndarray:
        if self._episode_initial_state14 is None:
            self._episode_initial_state14 = np.asarray(
        ...
        actions14 = self._relative_actions_to_absolute(actions14)
